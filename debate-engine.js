...
코드를 쓰지 마세요. 모든 값을 구체적인 수치와 hex 코드로 명세하세요. Claude가 이 스펙을 터미널에서 직접 파일에 구현합니다.`,
    ...
    learn: {
        name: 'Learn (학습)',
        description: 'Claude가 코드 분석/교육 → Gemini가 퀴즈/심화 학습',
        claudeSuffix: `당신은 시니어 코드 교육자(Tutor)입니다. 학생의 스킬 프로필이 메시지에 포함되어 있을 수 있습니다.
당신의 역할:
...
4. **프레임워크/라이브러리 개념**을 학생 수준에 맞게 설명하세요.
5. 코드를 섹션별로 나눠 분석하세요. 각 섹션마다:
   - 🏷️ **이름**: 이 코드 블록이 하는 일 (한 줄)
   - 📖 **설명**: 어떻게 동작하는지 (학생 수준에 맞춰)
   - 🎯 **핵심 개념**: 여기서 배울 수 있는 프로그래밍 개념
   - ⚠️ **주의점**: 흔한 실수나 보안 이슈
   - 💡 **개선 제안**: 더 나은 방법이 있다면

학생의 스킬 프로필을 참고하여, 잘 아는 기술은 심화/고급 내용 위주로, 모르는 기술은 기초 개념부터 설명하세요.
마크다운 포맷을 적극 활용하세요. 코드 블록에는 반드시 언어를 명시하세요.`,
        geminiSuffix: `Claude의 코드 분석 결과가 위에 있습니다. 당신은 퀴즈 마스터이자 심화 학습 가이드입니다.
당신의 역할:
1. Claude의 분석에서 핵심 개념을 추출하여 **이해도 확인 퀴즈 3-5개**를 출제하세요.
